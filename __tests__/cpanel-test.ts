...
import { CPanelAccess } from "../lib/panels/50-cpanel.ts";
...
describe('cPanelAccess', () => {
    ...
    test('isActive is false when missing /usr/local/cpanel/version file', async () => {
    ...
    test('findDomainLogFiles, addon domain', async () => {
        const actual = await c.findDomainLogFiles('alscan.org');
        const expected: Array<ExpectedRow> = [
    ...
    test('findDomainLogFiles, subdomain', async () => {
        const actual = await c.findDomainLogFiles('pub.samplx.org');
        const expected: Array<ExpectedRow> = [
    ...
    test('findDomainLogFiles, nonexistent domain', async () => {
    ...
    test('findAccountLogFiles("druid")', async () => {
        const actual = await c.findAccountLogFiles('druid');
        const expected: Array<ExpectedRow> = [
            { filename:  '/usr/local/apache/domlogs/alscan-org.druiddesigns.com', domain: 'alscan.org' },
            { filename:  '/usr/local/apache/domlogs/druiddesigns.com', domain: 'druiddesigns.com' },
            { filename:  '/usr/local/apache/domlogs/druiddesigns.com-ssl_log', domain: 'druiddesigns.com' },
    ...
    test('findAccountLogFiles("alscan")', async () => {
        const actual = await c.findAccountLogFiles('alscan');
        const expected: Array<ExpectedRow> = [
    ...
    test('findAccountLogFiles("nonesuch")', async () => {
        const actual = await c.findAccountLogFiles('nonesuch');
        const expected: Array<ExpectedRow> = [
        ];
        checkPromise(actual, expected);
    });
    ...
    test('findDomainArchiveFiles("druiddesigns.com", "2013/08/12", "2013/09/21")', async () => {
        ...
        var actual = await c.findDomainArchiveFiles('druiddesigns.com', start, stop);
    ...
    test('findDomainArchiveFiles("alscan.info")', async () => {
        const start = new Date(2001, 0, 1);
        const stop  = new Date();
        const actual = await c.findDomainArchiveFiles('alscan.info', start, stop);
        const expected: Array<ExpectedRow> = [
        ];
        checkPromise(actual, expected);
    });

    test('findDomainArchiveFiles("samplx.org")', async () => {
        const start = new Date(2001, 0, 1);
        const stop  = new Date();
        const actual = await c.findDomainArchiveFiles('samplx.org', start, stop);
        const expected: Array<ExpectedRow> = [
        ];
        checkPromise(actual, expected);
    });
    ...
    test('findAccountArchiveFiles("samplx", "2013/09/12", "2013/09/21")', async () => {
        const start = new Date(2013, 8, 12, 0, 0, 0, 0);
        const stop = new Date(2013, 8, 21, 23, 59, 59, 0);
        const actual = await c.findAccountArchiveFiles('samplx', start, stop);
        const expected: Array<ExpectedRow> = [
        ];
        checkPromise(actual, expected);
    });

    test('findAccountArchiveFiles("alscan", "2013/09/12", "2013/09/21")', async () => {
        const start = new Date(2013, 8, 12, 0, 0, 0, 0);
        const stop = new Date(2013, 8, 21, 23, 59, 59, 0);
        const actual = await c.findAccountArchiveFiles('alscan', start, stop);
        const expected: Array<ExpectedRow> = [
        ];
        checkPromise(actual, expected);
    });

    test('findAccountArchiveFiles("nonesuch", "2013/09/12", "2013/09/21")', async () => {
        const start = new Date(2013, 8, 12, 0, 0, 0, 0);
        const stop = new Date(2013, 8, 21, 23, 59, 59, 0);
        const actual = await c.findAccountArchiveFiles('nonesuch', start, stop);
        const expected: Array<ExpectedRow> = [
        ];
        checkPromise(actual, expected);
    });

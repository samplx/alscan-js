    ...
    describe('constructor creates keys', () => {
        ...
        test('all expected keys are in found set', () => {
            ...
            for (const eKey of expectedKeys) {
                assert.ok(keys.includes(eKey));
            }
        });
    });

    describe('parse', () => {
        const checkParse = (expectedResult: Record<string, unknown>) => {
            const a = new AccessLogEntry();
            if (typeof expectedResult['line'] == 'string') {
